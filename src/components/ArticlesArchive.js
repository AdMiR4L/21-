    ...
    return (
        ...
        <section className="articles articles-archive">
            <div className="container">
                ...
                <div className="row">
                    {loading ?
                        <div className="d-flex flex-nowrap w-100 mt-3 mb-4 pr-4">
                            {Array.from({length: 5}).map((_, index) => {
                                return <div key={index} className="cat-item ml-3">
                                    <Skeleton borderRadius={10} width={"90px"} height={"27px"}/>
                                </div>
                            })}
                        </div>
                                ...
                                >
                                    ...
                                    <SwiperSlide className="mobile-article">
                                        <Link to="/articles/archive?page=1"
                                              className={`cat-item ${searchParams.get('category') === null ? "active" : null}`}>
                                            همه
                                        </Link>
                                    </SwiperSlide>
                                    {categories.map((item, index) => {
                                        return <SwiperSlide className="mobile-article" key={index}>
                                            <Link to={`/articles/archive?page=1&category=${item.slug}`} key={index}
                                                  className={`cat-item ${searchParams.get('category') === item.slug ? "active" : null}`}>
                                                {item.title}
                                            </Link>
                                        </SwiperSlide>
                                    })}
